...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (state?.registered) {
      setSuccessMessage(state.message ?? 'Kayıt başarılı. Giriş yapabilirsiniz.');
    ...
    try {
      ...
      if (isApiError(error)) {
        if (error.code === 'INVALID_CREDENTIALS' || error.status === 401) {
          setFormError(error.message || 'E-posta veya şifre hatalı.');
          ...
          setFormError(error.message || 'Giriş yapılamadı. Lütfen tekrar deneyin.');
        }
      } else {
